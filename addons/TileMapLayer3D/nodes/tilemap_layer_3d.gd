...
func _update_material() -> void:
	if tileset_texture:
		...
		# Update material on all arch-two chunks (single-sided like FLAT_SQUARE)
...
func _create_chunk_config(mesh_mode: GlobalConstants.MeshMode, texture_repeat: int) -> ChunkConfig:
	...
	match mesh_mode:
		...
		GlobalConstants.MeshMode.FLAT_ARCH_TWO:
			...
			config.registry = _chunk_registry_arch_two
			config.flat_array = _arch_two_chunks
			config.name_prefix = "ArchTwoChunk"
			config.needs_double_sided = false
...
func rebuild_arch_chunk_meshes() -> void:
	...
	for chunk in _arch_two_chunks:
		if chunk and chunk.multimesh:
			chunk.multimesh.mesh = new_arch_two_mesh
